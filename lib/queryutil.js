...
Util = {
  ...
  whereString : function(field, values, escape) {
    var sql = '';
    // check array or not
  ...
  createQuery : function(entity, records, query) {
    if (!query) {
    ...
    if (records) {
      ...
      for (var field in keys) {
        ...
        if (values.length > 0) {
          ...
          que = Util.whereString(field, values, escape);
          queries.push(que);
        }
      }
    }
  ...
  createQueryWithParams : function(entity, params, query) {
    if (!query) {
      if (!entity || !params) {
        return;
      }
